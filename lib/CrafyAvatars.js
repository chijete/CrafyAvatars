...
class CrafyAvatars {
    ...
    generate(base_array) {
        ...
        for (const [shape_key, shape_val] of Object.entries(shapes)) {
            ...
            var splited_svg_string = svg_string.split(/:#(.*?);/);

            var colors_index = 0;
            var key = 0;

            var final_svg_string = '';

            for (const value of splited_svg_string) {
                if (key % 2 == 0) { // par
                    final_svg_string += value;
                } else { // impar
                    final_svg_string += ':' + svg_colors[colors_index] + ';';
                    colors_index++;
                }
                key++;
            }
