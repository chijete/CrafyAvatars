...
class CrafyAvatars
{
    ...
    function __construct()
    {
        ...
        $this->shapes_end_key = (int) key($this->shapes);

        end($this->themes['00']);
        $this->colors_end_key = (int) key($this->themes['00']);
    ...
    */
    ...
    public function generate($base_array)
    {
        $shapes = [
            'env' => $this->env,
            'head' => $this->head,
        ...
        foreach ($shapes as $shape_key => $shape_val) {
            $svg_string = $shape_val;
            $svg_colors = $colors[$shape_key];
            if (preg_match_all('/#(.*?)+(?=;)/', $svg_string, $result)) {
                $i = 0;
                foreach ($result[0] as $var) {
                    $pos = strpos($svg_string, $var);
                    if ($pos !== false) {
                        $svg_string = substr_replace($svg_string, $svg_colors[$i], $pos, strlen($var));
                    }
                    $i++;
                }
            }
            $svg_array[] = $svg_string;
        }

        return $svg_array;
    }

    public function generate_svg($base_array)
    {
        $svg_parts = $this->generate($base_array);
        $final_svg = '';
        $final_svg .= $this->svgStart;
        foreach ($svg_parts as $svgPart) {
            $final_svg .= $svgPart;
        }
    ...
    public function generate_svg_byInt($base_int)
    {
        ...
        if (count($base_array) == 10) {
            ...
            if (
                ...
                intval($base_array[3]) <= $this->shapes_end_key and
                intval($base_array[4]) <= $this->colors_end_key and
                ...
                intval($base_array[1]) >= 0 and
                intval($base_array[2]) >= 0 and
                intval($base_array[3]) >= 0 and
                intval($base_array[4]) >= 0 and
                intval($base_array[5]) >= 0 and
                intval($base_array[6]) >= 0 and
                intval($base_array[7]) >= 0 and
                intval($base_array[8]) >= 0 and
                intval($base_array[9]) >= 0
            ) {
    ...
    public function getAleatoryAvatarID()
    {
        $base_array = [];

        $base_array[] = random_int(0, $this->shapes_end_key);
        $base_array[] = random_int(0, $this->shapes_end_key);
        $base_array[] = random_int(0, $this->shapes_end_key);
        $base_array[] = random_int(0, $this->shapes_end_key);
        ...
        foreach ($base_array as $baseElement) {
            if ($baseElement < 10) {
                $base_string .= '0' . $baseElement;
            } else {
                $base_string .= (string) $baseElement;
            }
        }
